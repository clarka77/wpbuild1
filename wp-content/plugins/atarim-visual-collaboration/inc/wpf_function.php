    ...
    function wp_feedback_get_texonomy( $my_term ) {
        ...
        if ( isset( $filter_data_db ) && ! empty( $filter_data_db ) ) {
            ...
            foreach ( $filter_data_db as $term ) {
                ...
                echo '<li><input onclick="wp_feedback_filter()" type="checkbox" name="' . $my_term . '" value="' . $term['value'] . '" class="wp_feedback_task wpf_checkbox"  id="' . $term['value'] . '"/><label for="' . $term['value'] . '" class="wpf_checkbox_label">' . __( $term['label'], 'atarim-visual-collaboration' ) . '</label></li>';
            }
            echo '</ul>';
        }
    }
    ...
    function wpf_admin_notice_success() {
        if ( isset( $_GET['wpf_setting'] ) && $_GET['wpf_setting'] == 1 ) {
            ...
            <div class="notice notice-success is-dismissible">
                <p><?php _e( 'Your settings have been saved!', 'atarim-visual-collaboration' ); ?></p>
            </div>
        <?php }
        ...
        if ( isset( $_GET['wpf_setting'] ) && $_GET['wpf_setting'] == 0 ) {
            ...
            <div class="error notice is-dismissible">
                <p><?php _e( 'Your Settings not saved. ', 'atarim-visual-collaboration' ); ?></p>
            </div>
        <?php }
    }
}
        ...
        if ( $interval->y == 0 ) {
            if ( $interval->m == 0 ) {
                if ( $interval->d == 0 ) {
                    if ( $interval->h == 0 ) {
                        if ( $interval->i == 0 ) {
                            $comment_time = $interval->s . __( 'sec', 'atarim-visual-collaboration' );
                        } else {
                            $comment_time = $interval->i . __( 'min', 'atarim-visual-collaboration' );
                        }
                    } else {
                        $comment_time = $interval->h . __( 'hr', 'atarim-visual-collaboration' );
                    }
                } else {
                    $comment_time = $interval->d . __( 'd', 'atarim-visual-collaboration' );
                }
            } else {
                $comment_time = $interval->m . __( 'month', 'atarim-visual-collaboration' );
            }
        } else {
            $comment_time = $interval->y . __( 'year', 'atarim-visual-collaboration' );
        }
                ...
                <tr>
                    ...
                    <td>
                        <select id="wpf_user_type" name="wpf_user_type">
                            ...
                            <option value="king" <?php if ( $wpf_get_user_type == 'king' ) { echo 'selected'; } ?> ><?php echo ! empty( get_site_data_by_key( 'wpf_customisations_client' ) ) ? get_site_data_by_key( 'wpf_customisations_client' ) : 'Client (Website Owner)'; ?></option>
                            <option value="advisor" <?php if ( $wpf_get_user_type == 'advisor' ) { echo 'selected'; } ?> ><?php echo ! empty( get_site_data_by_key( 'wpf_customisations_webmaster' ) ) ? get_site_data_by_key( 'wpf_customisations_webmaster' ) : 'Webmaster'; ?></option>
                            <option value="council" <?php if ( $wpf_get_user_type == 'council' ) { echo 'selected'; } ?> ><?php echo ! empty( get_site_data_by_key( 'wpf_customisations_others' ) ) ? get_site_data_by_key( 'wpf_customisations_others' ) : 'Others'; ?></option>
                        </select>
                    </td>
                </tr>
                <tr>
...
function wpf_grapgics_version_list( $dataver, $post_id ) {
    ...
    if ( isset( $data['image'] ) && count( $data['image'] ) > 0 ) {
        ...
        foreach ( $data['image'] as $all_graphics_id ) {
            ...
            $output .= '<option name="wpf_graphics_id" value="' . $all_graphics_id['version'] . '" class="wpf_graphics_version"  data-src="' . $all_graphics_id['image'] . '" ' . $selected . '> ' . __( "Version", 'atarim-visual-collaboration' ) . " " . $all_graphics_id['version'] . ' </option>';
            $i++;
        }
    }    
    $output .= '</select>';
...
function avc_menubar() {
    $wpf_user_type = wpf_user_type();
    if ( $wpf_user_type != 'advisor' ) {
        return;
    }

    $menubar = '<div class="avc_menu_wrapper">
                    <div class="avc_menu_container">
                        <div class="avc_each_menu">
                            <a href="'. WPF_APP_SITE_URL .'/dashboard" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-dash.svg" alt="" /></a>
                        </div>
                        <div class="avc_each_menu">
                            <a href="'. WPF_APP_SITE_URL .'/projects" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-project.svg" alt="" /></a>
                        </div>
                        <div class="avc_each_menu">
                            <a href="'. WPF_APP_SITE_URL .'/tasks" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-inbox.svg" alt="" /></a>
                        </div>
                        <div class="avc_each_menu">
                            <a href="'. WPF_APP_SITE_URL .'/board" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-board.svg" alt="" /></a>
                        </div>
                        <div class="avc_each_menu">
                            <a href="'. WPF_APP_SITE_URL .'/forms" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-form.svg" alt="" /></a>
                            <span class="avc-menu-beta-tag">BETA</span>
                        </div>
                        <div class="avc_each_menu">
                            <a href="'. WPF_APP_SITE_URL .'/time-entries" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-timer.svg" alt="" /></a>
                        </div>
                        <div class="avc_each_menu">
                            <a href="'. WPF_APP_SITE_URL .'/settings" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-setting.svg" alt="" /></a>
                        </div>
                        <div class="avc_each_menu">
                            <a href="'. WPF_APP_SITE_URL .'/user" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-user.svg" alt="" /></a>
                        </div>
                        <div class="avc_each_menu">
                            <a href="https://support.atarim.io/" target="_blank"><img src="' . WPF_PLUGIN_URL . 'images/app-help.svg" alt="" /></a>
                        </div>
                    </div>
                </div>';
    return $menubar;
...
function generate_bottom_part_html() {
    ...
    if ( is_feature_enabled( 'bottom_bar_enabled' ) ) {
        ...
        if ( $current_user_id > 0 ) {
            ...
            if ( get_query_var( 'is_graphic_page' ) ) {
                $wpf_current_page_url = site_url() . '/collaborate/graphic?id=' . sanitize_text_field(  $_GET['id'] ) . '&wpf_login=1';
            }
            /* ================Go to dashboard Tabs HTML================ */
            $wpf_report_btn .= '<div class="wpf_report_trigger"><label class="wpf_reports_title">' . get_wpf_report_icon() . __( "Send Reports:", 'atarim-visual-collaboration' ) . '</label>';
            if ( $wpf_daily_report == 'yes' ) {
                /* ================Daily report btn HTML================ */
                $wpf_report_btn .= '<a href="javascript:wpf_send_report(\'daily_report\')">' . __( 'Last 24 Hours', 'atarim-visual-collaboration' ) . '</a>';
            }
            if ( $wpf_weekly_report == 'yes' ) {
                /* ================Weekly report btn HTML================ */
                $wpf_report_btn .= '<a href="javascript:wpf_send_report(\'weekly_report\')">' . __( 'Last 7 Days', 'atarim-visual-collaboration' ) . '</a>';
            }
            $wpf_report_btn .= '<span id="wpf_front_report_sent_span" class="wpf_hide text-success">' . __( 'Your report was sent', 'atarim-visual-collaboration' ) . '</span></div>';

            /* ================Report Tabs HTML================ */
            $wpf_report_btn_tab = '<li class="reports"><a class="wpf_filter_tab_btn_bottom" href="javascript:void(0);" data-tag="wpf_report_btn" title="' . __( "Reports", 'atarim-visual-collaboration' ) . '">' . get_wpf_report_icon() . ' <span>' . __( "Reports", 'atarim-visual-collaboration' ) . '</span></a></li>';
        } else {
            $share_style = 'style="margin-left: 90px;"';
        }
        
        /* ================share Tabs Content HTML================ */
        $wpf_page_share = '<div class="wpf_icon_title">'.get_wpf_share_icon(). __("Share Page Link : ", 'atarim-visual-collaboration') . '</div>
                           <input type="text" id="wpf_share_page_link" value="' . $wpf_current_page_url . '" style="position: absolute; z-index: -999; opacity: 0;">
                           <span class="wpf_share_task_link"><div class="wpf_task_link">' . $wpf_current_page_url . '</div><a href="javascript:void(0);" onclick="wpf_copy_to_clipboard(\'wpf_share_page_link\')" class="wpf_copy_task_icon" style="display: inline-block; color: var(--main-wpf-color) !important;"><i class="gg-copy"></i></a><span class="wpf_success_wpf_share_link" id="wpf_success_wpf_share_page_link" style="display: none;">The link was copied to your clipboard.</span></span>
        ...
        if ( $wpf_current_role == 'advisor' || ( $wpf_current_role == '' && current_user_can( 'administrator' ) ) ) {
            $agency_menu_item = '<li class="pro"> <a href="' . WPF_APP_SITE_URL . '/login" target="_blank" title="' . __( "Atarim Dashboard", 'atarim-visual-collaboration' ) . '">' . get_wpf_pro_icon() . '<span>' . __( "Agency", 'atarim-visual-collaboration' ) . '</span> </a></li>';
        }
        if ( (! get_query_var( 'is_graphic_page' ) ) && ( isset( $bottom_panel_db ) && $bottom_panel_db == '0' ) ) {
            $bottom_active_class = '';
            $bottom_style        = 'bottom: -60px;';
        }

        if ( get_query_var( 'is_graphic_page' ) ) {
            $toggle_button = "";
        } else {
            $toggle_button = '<a href="javascript:expand_bottom_bar()" class="arrow-down wpf_btn_hover" id="wpf_bottom_arrow" title="Hide Panel"><img src="' . WPF_PLUGIN_URL . 'images/arrow-down.svg" alt="arrow down image"></a>';
        }

        $app_button = '';
        if ( $wpf_current_role == 'advisor' || ( $wpf_current_role == '' && current_user_can( 'administrator' ) ) ) {
            $app_button = '<a href="javascript:expand_menubar()" id="wpf_app_dashboard" class="wpf_btn_hover" title="' . __( "Atarim Dashboard", 'atarim-visual-collaboration' ) . '"><img src="' . WPF_PLUGIN_URL . 'images/menu.svg" alt="atarim dashbord"></a>';
        }
        ...
        if( current_user_can( 'administrator' ) ) {
            ...
            if ( strpos( $current_page, '?' ) !== false) {
                ...
                $current_login_url = $current_page . '&wpf_login=1';
            } else {
                $page_token_url = $current_page . '/?collab_token=';
                $current_login_url = $current_page . '/?wpf_login=1';
            }
            ...
            $share_popup = '<div class="avc_invite_share_wrapper">
                                <div class="avc_invite_share_container">
                                    ...
                                    <div class="avc_invite_share_icon"><img src="'.WPF_PLUGIN_URL.'/images/share_icon.svg" ></div>
                                                ...
                                                $share_popup .= '<div class="avc_invited_user">
                                                                    <div class="avc_ivi_user_info">
                                                                        ...
                                                                        <div class="avc_user_share_action">
                                                                            <div class="avc_delete_ivi_user" data-id="' . $data["id"] . '">Remove</div>
                                    ...
                                    <div class="avc_share_page_link">
                                        ...
                                        <div class="avc_share_page_container">
                                            <div class="avc_share_page_icon"><img src="'.WPF_PLUGIN_URL.'/images/link.svg" ></div>
                                ...
                                <div class="avc_hidden_user_structure">
                                    <div class="avc_invited_user">
                                        <div class="avc_ivi_user_info">
                                            ...
                                            <div class="avc_ivi_user_name"><span></div>
                                            <div class="avc_user_share_action">
                                                <div class="avc_delete_ivi_user" data-id="">Remove</div>
                                                <div class="avc_share_button">
        ...
        $sidebar = '<a href="javascript:expand_sidebar()" class="tasks-btn wpf_btn_hover" title="Sidebar"> </span><img src="' . WPF_PLUGIN_URL . 'images/sidebar.svg" alt="sidebar icon"><span class="number wpf_hide" id="wpf_total_task_number"></span> </a>';
        /* ================visibility Tabs Content HTML================ */
        $visibility_conatiner = '<div class="wpf_visibility_container wpf_btn_hover">
                                    <img src="' . WPF_PLUGIN_URL . 'images/visible.svg" alt="visibility icon">
                                    <div id="wpf_task_visibility">
                                        <div class="wpf_sidebar_checkboxes"><input type="checkbox" name="wpfb_display_tasks" id="wpfb_display_tasks" class="wpf_checkbox" ' . $checkbox_checked . '/> <label for="wpfb_display_tasks" class="wpf_checkbox_label">' . __( 'Show Tasks', 'atarim-visual-collaboration' ) . '</label></div>
                                        <div class="wpf_sidebar_checkboxes"><input type="checkbox" name="wpfb_display_completed_tasks" class="wpf_checkbox" id="wpfb_display_completed_tasks" /> <label for="wpfb_display_completed_tasks" class="wpf_checkbox_label">' . __( 'Show Completed', 'atarim-visual-collaboration' ) . '</label></div>
                                        <div class="wpf_sidebar_checkboxes"><input type="checkbox" name="wpfb_display_internal_tasks" class="wpf_checkbox" id="wpfb_display_internal_tasks" checked /> <label for="wpfb_display_internal_tasks" class="wpf_checkbox_label">' . __( 'Show Internal', 'atarim-visual-collaboration' ) . '</label></div>
                                    </div>
                                </div>';
        $general_button = '<a class="wpf_general_btn wpf_comment_mode_general_task wpf_btn_hover" id="wpf_comment_mode_general_task" href="javascript:void(0)" onclick="wpf_new_general_task(0)" title="' . __( 'General', 'atarim-visual-collaboration' ) . '"><img src="' . WPF_PLUGIN_URL . 'images/general.svg" alt="general button icon"><span>' . __( 'General', 'atarim-visual-collaboration' ) . '</span></a>';
        $share_btn = '';
        if ( is_user_logged_in() ) {
            $share_btn = '<div class="wpf_share_btn wpf_btn_hover" href="javascript:void(0);" title="' . __( "Share", 'atarim-visual-collaboration' ) . '"><img src="' . WPF_PLUGIN_URL . 'images/share.svg" alt="share icon"><span>' . __( "Share", 'atarim-visual-collaboration' ) . '</span></div>';
        }

        $wpf_comment = '<a href="javascript:enable_comment();" class="wpf_green_btn wpf_comment_btn" title="Click to give your feedback!"><img src="' . WPF_PLUGIN_URL . 'images/comment.svg" alt="comment icon"><span>' . __( "Add Comment", 'atarim-visual-collaboration' ) . '</span></a>';
...
function generate_side_part_html() {
    ...
    /* ================visibility Tabs Content HTML================ */
    $wpf_task_visibility = '<label class="wpf_visibility_title">' . get_wpf_visibility_icon() . __( "Tasks Visibility", 'atarim-visual-collaboration' ) . '</label><div class="wpf_sidebar_checkboxes"><input type="checkbox" name="wpfb_display_tasks" id="wpfb_display_tasks" class="wpf_checkbox" ' . $checkbox_checked . '/> <label for="wpfb_display_tasks" class="wpf_checkbox_label">' . __( 'Show Tasks', 'atarim-visual-collaboration' ) . '</label></div>
            <div class="wpf_sidebar_checkboxes"><input type="checkbox" name="wpfb_display_completed_tasks" class="wpf_checkbox" id="wpfb_display_completed_tasks" /> <label for="wpfb_display_completed_tasks" class="wpf_checkbox_label">' . __( 'Show Completed', 'atarim-visual-collaboration' ) . '</label></div>';
    ...
    $approve_btn = '<button class="wpf_green_btn approve-page" title="' . __( "Approve Page", 'atarim-visual-collaboration' ) . '" data-is-approve="' . $is_approved . '">' . get_wpf_right_icon() . '<span>' . $btn_title . '</span> </button>';
    if ( is_admin() == 1 ) {
        $approve_btn = "";
    }
