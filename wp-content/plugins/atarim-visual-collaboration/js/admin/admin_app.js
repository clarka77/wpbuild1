...
function enable_comment(){
    ...
    /* Add text with the cursor text when user click on the "+ Comment" button on admin => v2.0.9 */
            ...
            var new_task_screenshot_obj = jQuery_WPF.extend({}, task_screenshot);
                ...
                jQuery_WPF.ajax({
                    ...
                    success: function(data){
                        ...
                        var comment_html = '<li class="wpf_author"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + current_user_name + '</div><span>just now</span></level><div class="meassage_area_main"><a href="'+data+'" target="_blank"><img src="'+data+'" alt="" /></a></div></div></div></li>';
...
jQuery_WPF(document).ready(function(){
    if(disable_for_admin == 0){
        ...
        jQuery_WPF( function() {
            ...
            jQuery_WPF( "#wpf_launcher > #wpf_launch_buttons_wrapper" ).draggable({ containment: "#wpwrap" });
    });
    }
   ...
   jQuery_WPF.ajax({
        ...
        success : function(data){
            ...
            if(task_status == 'complete'){
                ...
                jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'],#wpf_allpages [data-taskid='" + id + "'],#wpf_frontend [data-taskid='" + id + "']").removeClass('open').removeClass('complete').removeClass('pending-review').removeClass('in-progress').removeClass('complete_custom').removeClass('open_custom').removeClass('pending-review_custom').removeClass('in-progress_custom');
                
                jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'],#wpf_allpages [data-taskid='" + id + "'],#wpf_frontend [data-taskid='" + id + "']").addClass(task_status+status_class_str);
                ...
                if(wpf_tab_permission.display_stickers == 'yes'){
                    ...
                    jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'] .wpf_task_number,#wpf_allpages [data-taskid='" + id + "'] .wpf_task_number,#wpf_frontend [data-taskid='" + id + "'] .wpf_task_number").html('<span class="sticker '+task_priority+' '+task_priority+'_custom"></span> '+display_check_mark);
                }else{
                    jQuery_WPF('#bubble-'+id).html(display_check_mark);
                    jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'] .wpf_task_number,#wpf_allpages [data-taskid='" + id + "'] .wpf_task_number,#wpf_frontend [data-taskid='" + id + "'] .wpf_task_number").html(display_check_mark);
                }
                ...
                if(jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'] .wpf_task_number, #wpf_allpages [data-taskid='" + id + "'] .wpf_task_number,#wpf_frontend [data-taskid='" + id + "'] .wpf_task_number").find('span .'+task_priority).length == 0 && wpf_tab_permission.display_stickers == 'yes'){
                    jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'] .wpf_task_number, #wpf_allpages [data-taskid='" + id + "'] .wpf_task_number,#wpf_frontend [data-taskid='" + id + "'] .wpf_task_number").html('<span class="'+task_priority+' '+task_priority+'_custom"></span>'+view_id_sidebar);
                }else{
                    jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'] .wpf_task_number, #wpf_allpages [data-taskid='" + id + "'] .wpf_task_number,#wpf_frontend [data-taskid='" + id + "'] .wpf_task_number").html(view_id_sidebar);
                }
                ...
                if(wpf_tab_permission.display_stickers == 'yes'){
                    jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'], #wpf_allpages [data-taskid='" + id + "'],#wpf_frontend [data-taskid='" + id + "']").addClass(task_status+' '+task_status+'_custom');    
                }else{
                    jQuery_WPF(document).find("#wpf_thispage [data-taskid='" + id + "'], #wpf_allpages [data-taskid='" + id + "'],#wpf_frontend [data-taskid='" + id + "']").addClass(task_status);
                }
...
function new_comment(id, internal=0, note=false){
    ...
    if(jQuery_WPF.trim(jQuery_WPF('textarea#comment-' + id).val()).length==0){
        ...
        // if the task is internal, it can create without any user
        if ( internal == 0 ) {
...
function generate_task(id, internal, note){
    ...
    if(wpf_is_valid_url_admin(strippedString) == true){
        if ( is_video_Url( strippedString ) ) {
            temp_task_text = wpf_is_valid_video_url_admin(strippedString);
        } else {
        ...
        success : function(data){ 
            ...
            if(data!=0){
                ...
                if(new_task['task_status'] == 'complete'){
                    ...
                    bubble_label = '<span class="wpf_bubble_num_wrapper">'+view_id+'</span>';//id;
                }
                ...
                if(new_task['task_type']=='general'){
                    ...
                    if(wpf_tab_permission.display_stickers == 'yes'){
                        jQuery_WPF('#wpf_thispage_container_today').prepend('<li class="current_page_general_task '+task_status+' '+task_status+'_custom" data-taskid="'+id+'" data-postid="'+tasks_on_page[id]+'"><div class="wpf_task_number" data-disp-id="'+view_id+'"> <span class="'+task_priority+'_custom"></span>'+bubble_label+'</div><div class="wpf_task_sum"><level class="task-author">'+current_user_name+'<span>'+wpf_just_now+'</span></level><div class="current_page_task_list">'+task_comment+'</div></div>'+internal_tag_html+'<span class="wpf_task_type" title="Task type">General</span><div class="wpf_task_meta"><div class="wpf_task_meta_icon"><i class="gg-chevron-left"></i></div><div class="wpf_task_meta_details"><span class="wpf_task_type">'+wpf_general_tag+'</span>'+wpf_task_status_label+wpf_task_priority_label+wpfb_tags_html+'</div></div></li>');
                    }else{
                        jQuery_WPF('#wpf_thispage_container_today').prepend('<li class="current_page_general_task '+task_status+'" data-taskid="'+id+'" data-postid="'+tasks_on_page[id]+'"><div class="wpf_task_number" data-disp-id="'+view_id+'">'+bubble_label+'</div><div class="wpf_task_sum"><level class="task-author">'+current_user_name+'<span>'+wpf_just_now+'</span></level><div class="current_page_task_list">'+task_comment+'</div></div>'+internal_tag_html+'<span class="wpf_task_type" title="Task type">General</span><div class="wpf_task_meta"><div class="wpf_task_meta_icon"><i class="gg-chevron-left"></i></div><div class="wpf_task_meta_details"><span class="wpf_task_type">'+wpf_general_tag+'</span>'+wpf_task_status_label+wpf_task_priority_label+wpfb_tags_html+'</div></div></li>');
                    }
                }else{
                    if(wpf_tab_permission.display_stickers == 'yes'){
                        jQuery_WPF('#wpf_thispage_container_today').prepend('<li class="current_page_task '+task_status+' '+task_status+'_custom" data-taskid="'+id+'"><div class="wpf_task_number" data-disp-id="'+view_id+'"><span class="'+task_priority+'_custom"></span>'+bubble_label+'</div><div class="wpf_task_sum"><level class="task-author">'+current_user_name+' '+wpf_just_now+' </level><div class="current_page_task_list">'+task_comment+'</div></div>'+internal_tag_html+'<div class="wpf_task_meta"><div class="wpf_task_meta_icon"><i class="gg-chevron-left"></i></div><div class="wpf_task_meta_details">'+wpf_task_status_label+wpf_task_priority_label+wpfb_tags_html+'</div></div></li>');
                    }else{
                        jQuery_WPF('#wpf_thispage_container_today').prepend('<li class="current_page_task '+task_status+'" data-taskid="'+id+'"><div class="wpf_task_number" data-disp-id="'+view_id+'">'+bubble_label+'</div><div class="wpf_task_sum"><level class="task-author">'+current_user_name+' '+wpf_just_now+' </level><div class="current_page_task_list">'+task_comment+'</div></div>'+internal_tag_html+'<div class="wpf_task_meta"><div class="wpf_task_meta_icon"><i class="gg-chevron-left"></i></div><div class="wpf_task_meta_details">'+wpf_task_status_label+wpf_task_priority_label+wpfb_tags_html+'</div></div></li>');
                    }
                }
                img_dwn_icon = "";
        ...
        success: function(data){
            ...
            try {
                ...
                if ( responseData['limit'] === true ) {
                    ...
                    jQuery(".wpf-le-pop-wrapper").show();
                    jQuery_WPF('#wpf_error_'+id).hide();
                }
            } catch(ex){}
            jQuery_WPF('#wpf_error_'+id).hide();
...
function load_wpfb_tasks_admin(){
    ...
    jQuery_WPF.ajax({
        method:"POST",
        url: ajaxurl,
        data: {action:'load_wpfb_tasks_admin',wpf_nonce:wpf_nonce,wpf_current_screen:wpf_current_screen,all_page:0,task_page_url:current_page_url,current_page_id:current_page_id},
        success: function(data){
	    ...
	    jQuery_WPF('.wpf_page_loader').hide();
            ...
            jQuery_WPF(document).find("#wpf_frontend_container_today,#wpf_frontend_container_yesterday,#wpf_frontend_container_this_week,#wpf_frontend_container_this_month,#wpf_frontend_container_year,#wpf_frontend_container_other").on("click","li.current_page_task",function(e) {
                var task_url = jQuery_WPF(this).attr('data-task_url');
            ...
            jQuery_WPF(document).find("#wpf_frontend_container_today,#wpf_frontend_container_yesterday,#wpf_frontend_container_this_week,#wpf_frontend_container_this_month,#wpf_frontend_container_year,#wpf_frontend_container_other").on("click","li.current_page_general_task",function(e) {
                var taskid = jQuery_WPF(this).data('postid');
                wpf_load_general_task(taskid);
            });
    ...
    if(wpfb_metas.task_type=='general'){
        ...
        for (var key in wpfb_metas.comments) {
            ...
            if(wpfb_metas.comments[key].comment_type==0){
                ...
                let img_dwn_icon = "";
                wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + author + note_html + '</div><span>' + edited_html + wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main"><iframe width="100%" height="120" src="https://www.youtube.com/embed/'+wpfb_metas.comments[key].message+'"></iframe></div></div></div></li>';
            }
            else if (wpfb_metas.comments[key].comment_type==1) {
                wpfb_messages_html += '<li class="'+task_author+' is_image" data-comment_id="'+key+'"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + author + note_html + '</div><span>' + edited_html + wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main"><a href="'+wpfb_metas.comments[key].message+'" target="_blank">'+'</a>'+img_dwn_icon+'<img src="'+wpfb_metas.comments[key].message+'" alt="uploaded image" class="wpf_task_uploaded_image" /></div></div></div></li>';
            }
            else{
                let img_dwn_icon = "";
                var wpf_download_file  = wpfb_metas.comments[key].message.split("/").pop();
                wpfb_messages_html += '<li class="'+task_author+' is_file" data-comment_id="'+key+'"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + author + note_html + '</div><span>' + edited_html + wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main"><a href="'+wpfb_metas.comments[key].message+'" download="'+wpf_download_file+'" target="_blank"><i class="gg-software-download"></i> '+ wpf_download_file+'</a></div></div></div></li>';
            }
        ...
        if ( (wpfb_metas.task_type === 'element') && (jQuery_WPF(document).find(wpfb_metas.wpfb_task_bubble).length === 0) ) {
            wpfb_current_page_task_list_html = '<li class="current_page_task ' + wpfb_metas.task_status + ' ' + custom_status_class + ' ' + wpfb_metas.task_priority + '" data-taskid="' + comment_count + '" data-postid="' + wpfb_task_id + '"><div data-disp-id="' + wpfb_metas.site_task_id + '" class="wpf_task_number" title="' + wpf_Remap_text + '">' + bubble_label + '</div><div class="wpf_task_sum"><level class="task-author">' + wpfb_metas.task_config_author_name + '<span>' + wpfb_metas.task_time + '</span><span class="note-tooltip" data-note-tooltip="'+ note +'">!</span></level><div class="current_page_task_list">' + wpfb_metas.task_title + '</div><p class="note_task_become_general"></p></div>'+internal+'<div class="wpf_task_meta"><div class="wpf_task_meta_icon"><i class="gg-chevron-left"></i></div><div class="wpf_task_meta_details">' + wpf_task_status_label + wpf_task_priority_label + wpfb_tags_html + '</div></div></li>';
        } else {
...
function wpf_generate_general_task_html(wpfb_task_id,wpfb_metas){
    ...
    for (var key in wpfb_metas.comments) {
        ...
        if(wpfb_metas.comments[key].comment_type==0){
            if ( wpfb_metas.comments[key].is_note != true || (  wpfb_metas.comments[key].is_note == true && wpfb_metas.comments[key].wpf_user_id == wpfb_metas.current_user_id ) ) {
                wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + author + note_html + '</div><span>' + edited_html + wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main"><div id="wpf-chat-text-'+key+'" class="chat_text">'+ wpfb_metas.comments[key].message +'</div></div><div id="wpfb-edit-comment-wrapper-'+key+'" class="wpfb-edit-comment-wrapper"><div class="wpf-editor"></div><textarea class="form-control wpfb-edit-comment" data-comment_id="'+key+'" placeholder="Edit the comment..." spellcheck="false">'+URLify(wpfb_metas.comments[key].message)+'</textarea><button class="wpf_edit_comment_btn" onclick="wpfb_edit_comment('+key+')">'+edit_comment_text+'</button><a class="wpf-cancel-edit-comment" onclick="wpfb_cancel_edit_comment('+key+')" href="javascript:void(0)">'+cancel_edit_comment_text+'</a><div class="wpf_update_error wpf_hide">Please post your comment before performing this action</div></div></div></div></li>';
            }
        }
        else if (wpfb_metas.comments[key].comment_type==1) {
            wpfb_messages_html += '<li class="'+task_author+' is_image" data-comment_id="'+key+'"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + author + '</div><span>' + wpfb_metas.comments[key].time + '</span></level><div class="meassage_area_main"><a href="'+wpfb_metas.comments[key].message+'" target="_blank">'+'</a>'+img_dwn_icon+'<img src="'+wpfb_metas.comments[key].message+'" alt="uploaded image" class="wpf_task_uploaded_image" /></div></div></div></li>';
        }
        else if (wpfb_metas.comments[key].comment_type==3) {
            ...
            wpfb_messages_html += '<li class="'+task_author+' is_image" data-comment_id="'+key+'"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + author + '</div><span>' + wpfb_metas.comments[key].time + '</span></level><div class="meassage_area_main"><iframe width="100%" height="120" src="https://www.youtube.com/embed/'+wpfb_metas.comments[key].message+'"></iframe></div></div></div></li>';
        }
        else{
            var wpf_download_file  = wpfb_metas.comments[key].message.split("/").pop();
            wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + author + '</div><span>' + wpfb_metas.comments[key].time + '</span></level><div class="meassage_area_main"><a href="'+wpfb_metas.comments[key].message+'" download="'+wpf_download_file+'" target="_blank" ><i class="gg-software-download"></i> '+ wpf_download_file+'</a></div></div></div></li>';
        }
        jQuery_WPF('#task_comments_'+comment_count).append(wpfb_messages_html);
...
function wpf_task_popover_html(wpf_task_type, comment_count, wpfb_task_id, wpfb_metas, wpfb_users_html, wpfb_task_priority_html, wpfb_task_status_html, wpfb_messages_html,new_task_popover) {
	...
	if (wpf_task_type == 1) {
        ...
        if(wpfb_metas.is_internal=='1'){
            ...
            var button_html = '<a id="bubble-' + comment_count + '" data-html2canvas-ignore="true" href="javascript:void(0)" rel="popover-' + comment_count + '" data-html="true" data-trigger="focus" data-popover-content="#popover-content-c' + comment_count + '" class=" wpfb-internal wpfb-point ' + wpfb_metas.task_status + ' ' + wpf_hide_class + ' ' + custom_class + '" style="top:' + element_center['top'] + 'px;left:' + element_center['left'] + 'px;" data-toggle="popover" data-disp-id="'+wpfb_metas.site_task_id+'">' + display_sticker +bubble_label + '<span class="wpf_chevron_wrapper"><img src="' + plugin_url + 'images/eye-off-white.svg" alt="eye off white" class="wpf-internal-img"></span></a>';//+ bubble_label + '</a>';

        }else{
            var button_html = '<a id="bubble-' + comment_count + '" data-html2canvas-ignore="true" href="javascript:void(0)" rel="popover-' + comment_count + '" data-html="true" data-trigger="focus" data-popover-content="#popover-content-c' + comment_count + '" class="wpfb-point ' + wpfb_metas.task_status + ' ' + wpf_hide_class + ' ' + custom_class + '" style="top:' + element_center['top'] + 'px;left:' + element_center['left'] + 'px;" data-toggle="popover" data-disp-id="'+wpfb_metas.site_task_id+'">' + display_sticker + bubble_label + '</a>';
        }
	}
    }
	...
	tab_nav_html += '<li class="nav-item" title="Status"><a class="nav-link wpf_prio_tab" id="wpfbpriority-tab-' + comment_count + '" data-toggle="tab" href="#wpfbpriority-' + comment_count + '" role="tab" aria-controls="wpfbpriority" aria-selected="false">' + priority_icon + '</a></li>';
    }
    if (wpf_tab_permission.status == 'yes') {
	tab_nav_html += '<li class="nav-item" title="Priority"><a class="nav-link wpf_stat_tab" id="wpfbtaskstatus-tab-' + comment_count + '" data-toggle="tab" href="#wpfbtaskstatus-' + comment_count + '" role="tab" aria-controls="wpfbtaskstatus" aria-selected="false">' + status_icon + '</a></li>';
    }
	...
	tab_nav_html += '<li class="nav-item" title="Additional Information"><a class="nav-link wpf_deta_tab" id="wpfbsysinfo-tab-' + comment_count + '" data-toggle="tab" href="#wpfbsysinfo-' + comment_count + '" role="tab" aria-controls="wpfbsysinfo" aria-selected="false">' + info_icon + '</a></li>';
    }
    ...
    var tabs_html = '<div class="tab-content scrollbar-outer" id="myTabContent-' + comment_count + '">';
	...
	if (wpfb_task_id != 0) {
        ...
        if (wpfb_task_id != 0) {
            if ( current_page_url.lastIndexOf('?') >= 0 ) {
                task_link = current_page_url + '&wpf_taskid=' + comment_count + '&wpf_login=1';
            } else {
                task_link = current_page_url + '?wpf_taskid=' + comment_count + '&wpf_login=1';
            }
        } else {
	...
	wpf_screenshot = '<a target="_blank" href="#" class="wpf_screenshot_img_link"><img src="#" id="screenshot_img_' + comment_count + '" alt="screenshot" style="margin-top:10px; border:1px solid #dee2e6;padding: 5px; width:100%; display: none;" /></a>';
    }
	...
	if (wpf_tab_permission.display_stickers == 'yes') {
	    ...
	    tabs_html += '<div class="tab-pane" id="wpfbpriority-' + comment_count + '" role="tabpanel" aria-labelledby="wpfbpriority-tab"><div class="anim-slider"><input id="priority_low-' + comment_count + '" type="radio" name="wpfbpriority' + comment_count + '" data-elemid="' + comment_count + '" value="low" class="wpfbpriority " checked><label for="priority_low-' + comment_count + '" class="">' + wpf_priority_low + '</label><input id="priority_medium-' + comment_count + '" type="radio" name="wpfbpriority' + comment_count + '" data-elemid="' + comment_count + '" value="medium" class="wpfbpriority "><label for="priority_medium-' + comment_count + '" class="">' + wpf_priority_medium + '</label><input id="priority_high-' + comment_count + '" type="radio" name="wpfbpriority' + comment_count + '" data-elemid="' + comment_count + '" value="high" class="wpfbpriority "><label for="priority_high-' + comment_count + '" class="">' + wpf_priority_high + '</label><input id="priority_critical-' + comment_count + '" type="radio" name="wpfbpriority' + comment_count + '" data-elemid="' + comment_count + '" value="critical" class="wpfbpriority "><label for="priority_critical-' + comment_count + '" class="">' + wpf_priority_critical + '</label></div></div>';
	    tabs_html += '<div class="tab-pane" id="wpfbtaskstatus-' + comment_count + '" role="tabpanel" aria-labelledby="wpfbtaskstatus-tab"><div class="anim-slider"><input id="status_open-' + comment_count + '" type="radio" name="wpfbtaskstatus' + comment_count + '" value="open" data-elemid="' + comment_count + '" class="wpfbtaskstatus " checked><label for="status_open-' + comment_count + '" class="">' + wpf_status_open_task + '</label><input id="status_progress-' + comment_count + '" type="radio" name="wpfbtaskstatus' + comment_count + '" value="in-progress" data-elemid="' + comment_count + '" class="wpfbtaskstatus " ><label for="status_progress-' + comment_count + '" class="">' + wpf_status_in_progress + '</label><input id="status_pending-' + comment_count + '" type="radio" name="wpfbtaskstatus' + comment_count + '" value="pending-review" data-elemid="' + comment_count + '" class="wpfbtaskstatus " ><label for="status_pending-' + comment_count + '" class="">' + wpf_status_pending_review + '</label><input id="status_complete-' + comment_count + '" type="radio" name="wpfbtaskstatus' + comment_count + '" value="complete" data-elemid="' + comment_count + '" class="wpfbtaskstatus " ><label for="status_complete-' + comment_count + '" class="">' + wpf_status_complete + '</label></div></div>';
	}
...
function load_all_page_tasks_admin(){
    jQuery_WPF.ajax({
        method:"POST",
        url: ajaxurl,
        data: { action:'load_wpfb_tasks_admin', wpf_nonce: wpf_nonce, wpf_current_screen: wpf_current_screen, all_page: 1, page_no: page_no },
        beforeSend: function(){
            if ( page_no <= 1 ) {
        ...
        jQuery_WPF.ajax({
            ...
            success: function (data) {
                ...
                if(response.status==1){
                    ...
                    if(response.type==1){
                        img_dwn_icon  = "<span id='wpf_push_media' class='wpf_push_media wpf_image_download'>"+push_to_media_icon+"</span><span id='wpf_image_download' class='wpf_image_download' onclick='wpf_image_download_action(this)'>"+image_download_icon+"</span><span id='' class='wpf_image_open' onclick='wpf_image_open_new_tab(this)'>"+image_open_icon+"</span><span class='wpf_image_delete' onclick='wpf_task_image_delete(this)'>"+image_close_icon+"</span>";
                        var comment_html = '<li class="wpf_author is_image" data-comment_id="'+response.comment_id+'"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">' + current_user_name + '</div><span>just now</span></level><div class="meassage_area_main">'+img_dwn_icon+'<a target="_blank" href="'+response.message+'"><img src="'+response.message+'" alt=""></a></div></div></div></li>';
                    }
                    else{
                        var wpf_download_file  = response.message.split("/").pop();
                        var comment_html = '<li class="wpf_author"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="task-author"><div class="author-name">'+current_user_name+' '+wpf_just_now+'<span>just now</span></level><div class="meassage_area_main">'+img_dwn_icon+'<a href="'+response.message+'" download="'+wpf_download_file+'" target="_blank"><i class="gg-software-download"></i> '+ wpf_download_file+'</a></div></div></div></li>';
                    }
...
function load_all_frontend_wpfb_tasks(){
    jQuery_WPF.ajax({
        method:"POST",
        url: ajaxurl,
        data: {action:'load_wpfb_tasks',wpf_nonce:wpf_nonce,'from_admin':1},
        beforeSend: function(){
            jQuery_WPF('.wpf_sidebar_loader').show();
        },
        success: function(data){
	    ...
	    if(onload_wpfb_tasks != null && onload_wpfb_tasks != "null") {
            ...
            jQuery_WPF.each(k,function (index, value) {
                ...
                if(wpf_tab_permission.display_task_id != 'yes'){
                    ...
                    display_check_mark = onload_wpfb_tasks[value].site_task_id;//task_comment_id;
                }
                ...
                jQuery_WPF("#wpf_frontend_container"+sidebar_class).html(wpfb_all_page_task_list_html);
