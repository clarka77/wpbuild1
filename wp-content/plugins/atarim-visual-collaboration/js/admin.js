...
jQuery_WPF(document).ready(function () {
   ...
   /*STEP 1 BUTTON*/
    jQuery_WPF('.wpf_collaborator_user_container').show();
    jQuery_WPF('.wpf_collaborator_user_assign_container').hide();
    jQuery_WPF('.wpf_collaborator_setting').on('change', function() {
        if(jQuery_WPF(this).is(":checked")) {
            jQuery_WPF('.wpf_collaborator_user_container').hide();
            jQuery_WPF('.wpf_collaborator_user_assign_container').show();
        } else {
            jQuery_WPF('.wpf_collaborator_user_container').show();
            jQuery_WPF('.wpf_collaborator_user_assign_container').hide();
        }
    ...
    jQuery_WPF('#wpf_initial_setup_second_step_button').on('click',function (e) {
        var wpf_name = jQuery_WPF('.wpf_collaborator_name').val();
        var wpf_email = jQuery_WPF('.wpf_collaborator_email').val();
        var wpf_user = jQuery_WPF('.wp_feedback_filter_admin_user').val();
        var wpf_allow_guest = jQuery_WPF('#wpf_allow_guest:checkbox:checked').length;
        if(wpf_allow_guest > 0) {
            wpf_allow_guest = 'yes';
        } else {
            wpf_allow_guest = 'no';
        }
        if(jQuery_WPF('.wpf_collaborator_setting:checkbox:checked').length > 0) var collab = 'existing';
        else var collab = 'new';
        jQuery_WPF.ajax({
            url:ajaxurl,
            method: 'POST',
            data:{action: 'wpf_update_collaborator_user', wpf_nonce: wpf_nonce, wpf_name: wpf_name, wpf_email: wpf_email, wpf_user: wpf_user, wpf_allow_guest: wpf_allow_guest, collab: collab},
            beforeSend: function(){
                jQuery_WPF('.wpf_loader_admin').show();
            },
    ...
    jQuery_WPF('.wpf_collaborator_setting').on('change', function() {
        if(jQuery_WPF('.wpf_collaborator_setting:checkbox:checked').length > 0) jQuery_WPF('#wpf_initial_setup_second_step_button').html('Proceed To The Final Step');
        else jQuery_WPF('#wpf_initial_setup_second_step_button').html('Create an Account');
    });
    ...
    //show upgrade notice popup on blocked compact mode setting by Pratap
    jQuery_WPF('#wpf_enabled_compact_mode').change(function(){
        if(jQuery_WPF(this).hasClass('blocked')) {
            jQuery_WPF('.wpf-uf-popup-image img').attr('src', plugin_url + '/images/compact-mode.png');
            jQuery_WPF('.wpf-uf-plan-title').text('Bottom bar');
            jQuery_WPF('.wpf-uf-plan-detail').html('Approve pages entire pages with auto complete on all open tasks and have more control over your tasks with the bottom bar.');
            jQuery_WPF('.wpf-uf-plan-link').attr('href', upg_url + '?&feature=bottom');
            jQuery_WPF('.wpf-uf-pop-wrapper').show();
    ...
    jQuery_WPF('.wpf_global_settings').change(function () {
        ...
        if(jQuery_WPF(this).hasClass('blocked')) {
            ...
            jQuery_WPF('.wpf-uf-plan-detail').html('Save time and make it easy to set up this website by applying settings from your Agency Dashboard with one click, including white labelling, notification settings and permissions.');
...
// Skip step 2 and 3 of plugin activation by Pratap
function wpf_skip_initial_setup( site_url ) {
    var wpf_allow_guest = 'no';
    jQuery_WPF.ajax({
        url:ajaxurl,
        method: 'POST',
        data:{
            action: 'wpf_update_collaborator_user',
            wpf_nonce: wpf_nonce,
            wpf_allow_guest: wpf_allow_guest,
            skip_setup: 'yes'
        },
        beforeSend: function(){
            jQuery_WPF('.wpf_loader_admin').show();
...
function wpf_initial_setup_done(site_url, wpf_user) {
    jQuery_WPF.ajax({
        url:ajaxurl,
        method: 'POST',
        ...
        success: function (data) {
            jQuery_WPF('.wpf_loader_admin').hide();
            if(data==1){
                if(wpf_user == 'wpf_new_user') window.location=site_url+'?wpf-user-flow=true';
                else window.location=site_url+'?wpf-existing-user-flow=true';
            }
        }
    });
}
        ...
        jQuery_WPF.ajax({
            ...
            success: function (data) {
                ...
                if(response.status==1){
                    ...
                    if(response.type==1){
                        comment_html = '<li class="chat_author" id=""><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="wpf-author"><div>' + author + '</div><span>just now</span></level><div class="task_text"><a href="'+response.message+'" target="_blank"></a><div class="tag_img" style="width: 275px;height: 183px;"><div class="meassage_area_main"><a href="'+response.message+'" target="_blank"></a><img style="width: 100%;" class="wpfb_task_screenshot" src="'+response.message+'" alt="screenshot" />'+img_dwn_icon+'</div></div></div></div></div></li>';
                    }
                    else{
                        ...
                        comment_html = '<li class="chat_author"><div class="wpf-comment-container"><div class="wpf-author-img">' + author_html + '</div><div class="wpf-comment-wrapper"><level class="wpf-author"><div>' + author + '</div><span>just now</span></level><div class="task_text">'+response_message+'</div></div></div></li>';
                    }
