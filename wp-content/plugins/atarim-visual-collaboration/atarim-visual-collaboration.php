<?php
/*
 * Plugin Name: Atarim - Client Interface
...
function wpb_admin_notice_warn() {
    ...
    $is_notice = get_option('wp_feedback_notice', 'true');
    if ( ! $hide && current_user_can( 'administrator' ) && $wpf_user_type == 'advisor' && $is_notice == 'true' && ! isset( $_GET['page'] ) ) {
    ?>
        <div class="notice notice-warning wpf_admin_notice wpf-gftool-notice is-dismissible">
                <div class="wpf_admin_notice_icon">
                    <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1080 1080">
                        ...
                        <title><?php _e( 'Atarim Logo Inverted', 'atarim-visual-collaboration' ); ?></title>
                        <g>
                            <g>
                ...
                <div class="wpf_admin_notice_content">
                    <h4><?php _e( 'The Graphic Feedback Tool is moving to the Atarim Dashboard!', 'atarim-visual-collaboration' ); ?></h4>
                    ...
                    <p class="admin_notice_footer"><i><?php _e( '* This notice is shown to you as the Webmaster.', 'atarim-visual-collaboration' ); ?></i></p>
                </div>
            </div>
        <?php
    }
...
function wp_feedback_admin_menu() {
    ...
    if ( array_intersect( $current_user->roles, $selected_roles ) || current_user_can( 'administrator' ) ) {
        ...
        add_submenu_page(
            $main_menu_id, __( 'Tasks Center', 'atarim-visual-collaboration' ), __( 'Tasks Center', 'atarim-visual-collaboration' ), 'read', 'wpfeedback_page_tasks', 'wpfeedback_page_tasks'
        );
        add_submenu_page(
            '', __( 'Graphics', 'atarim-visual-collaboration' ), __( 'Graphics', 'atarim-visual-collaboration' ), 'read', 'wpfeedback_page_graphics', 'wpfeedback_page_graphics'
        );
        if ( $wpf_user_type == 'advisor' || ( $wpf_user_type == '' && current_user_can( 'administrator' ) ) ) {
            add_submenu_page(
                $main_menu_id, __( 'Settings', 'atarim-visual-collaboration' ), __( 'Settings', 'atarim-visual-collaboration' ), 'read', 'wpfeedback_page_settings', 'wpfeedback_page_settings'
            );
        }
        if ( $wpf_user_type == 'advisor' || ( $wpf_user_type == '' && current_user_can( 'administrator' ) ) ) {
            add_submenu_page(
                $main_menu_id, __( 'Permissions', 'atarim-visual-collaboration' ), __( 'Permissions', 'atarim-visual-collaboration' ), 'read', 'wpfeedback_page_permissions', 'wpfeedback_page_permissions'
            );
        }
        if ( $wpf_user_type == 'advisor' ) {
            add_submenu_page(
                $main_menu_id, __( 'Integrations', 'atarim-visual-collaboration' ), __( 'Integrations', 'atarim-visual-collaboration' ), 'read', 'wpfeedback_page_integrate', 'wpfeedback_page_integrate'
            );
        }

        if ( $wpf_user_type == 'advisor' || ( $wpf_user_type == '' && current_user_can( 'administrator' ) ) ) {
            add_submenu_page(
                $main_menu_id, __( 'Support', 'atarim-visual-collaboration' ), __( 'Support', 'atarim-visual-collaboration' ), 'read', 'https://atarim.io/support-reachout'
            );
            add_submenu_page(
                $main_menu_id, __( 'Upgrade', 'atarim-visual-collaboration' ), __( 'Upgrade', 'atarim-visual-collaboration' ), 'read', WPF_MAIN_SITE_URL.'/upgrade'
            );
        }
    }
}
...
function wpf_setting_action_links( $links ) {
    $links[] = '<a href="' . esc_url( get_admin_url( null, 'admin.php?page=wpfeedback_page_settings&wpf_setting=1' ) ) . '">' . __( 'Settings', 'atarim-visual-collaboration' ) . '</a>';
    return $links;
}
...
// setup the updater.
$edd_updater = new EDD_SL_Plugin_Updater( WPF_EDD_SL_STORE_URL, __FILE__,
    array(
        ...
        if ( $user_id != false ) {
            ...
            if ( $result != false ) { // If user exist
                ...
                if ( ! isset( $_COOKIE['wordpress_avc_allow_guest'] ) ) {
                    setcookie( 'wordpress_avc_allow_guest', json_encode( $user ), time() + ( 86400 * 30 ), '/');
        ...
        if ( ( get_query_var( 'is_graphic_page' ) ) || ( $enabled_wpfeedback == 1 && ! $is_site_archived ) ) {
            ...
            if ( $wpf_check_page_builder_active == 0 ) {
                ...
                if ( get_query_var( 'is_graphic_page' ) ) {
                    ...
                    wp_localize_script( 'wpf-graphics-front-script', 'upgrade_url', array( 'url' => $upgrade_url, 'plugin_url' => WPF_PLUGIN_URL ) );
                    ...
                    wp_localize_script( 'wpf-graphics-front-script', 'blocked', $feature );
                    ...
                    wp_localize_script( 'wpf_app_script', 'logged_user', array( 'current_user' => $wpf_user_type, 'author_img' => $avatar_url, 'author' => $display_name ) );
                    ...
                    wp_localize_script( 'wpf_app_script', 'istheme', array( 'adjust' => $adjust ) );
                    ...
                    $upgrade_url = get_option( 'upgrade_url' );
                    wp_localize_script( 'wpf_app_script', 'upgrade_url', array( 'url' => $upgrade_url, 'plugin_url' => WPF_PLUGIN_URL ) );
    ...
    if ( isset( $_REQUEST['page'] ) ) {
        if ( $_REQUEST['page'] == 'wpfeedback_page_settings' || $_REQUEST['page'] == 'wpfeedback_page_tasks' || $_REQUEST['page'] == 'wpfeedback_page_integrate' || $_REQUEST['page'] == 'wpfeedback_page_upgrade' || $_REQUEST['page'] == 'wpfeedback_page_support' || $_REQUEST['page'] == 'wpfeedback_page_permissions' || $_REQUEST['page'] == 'wpfeedback_page_graphics' ) {
            ...
            <script type='text/javascript'>
                ...
                var internal_icon_html='<span class="wpf_chevron_wrapper"><img src="' + plugin_url + 'images/eye-off-white.svg" class="wpf-internal-img" alt="eye off white"></span>';
...
function show_wpf_comment_button() {
    ...
    if ( $current_user_id > 0 ) {
        if ( $wpf_current_role == 'advisor' ) {
            $wpf_go_to_cloud_dashboard_btn_tab = '<a href="' . WPF_APP_SITE_URL . '/login" target="_blank" class="wpf_filter_tab_btn cloud_dashboard_btn" title="' . __("Atarim Dashboard", '') . '">'.get_wpf_icon().'</a>';
        }
        ...
        if ( get_query_var( 'is_graphic_page' ) ) {
            $wpf_current_page_url = site_url() . '/graphic?id=' . sanitize_text_field( $_GET['id'] ) . '?wpf_login=1';
        } else {
    ...
    if ( $wpf_active == 1 && $wpf_check_page_builder_active == 0 && ( ! $is_site_archived ) ) {
        ...
        if ( $disable_for_admin == 0 ) {
            ...
            $wpf_task_priority_filter_btn = '<div id="wpf_filter_taskpriority" class=""><label class="wpf_filter_title">' . get_wpf_priority_icon() . ' ' . __("Filter by Priority:", '') . '</label>' . wp_feedback_get_texonomy_filter("task_priority") . '</div>';
            ...
            if ( is_feature_enabled( 'bottom_bar_enabled' ) && $enable_compact_mode !== 'yes' ) {
                ...
                if ( get_query_var( 'is_graphic_page' ) ) {
                    $wpf_sidebar_header = '<div class="wpf_sidebar_header ' . $sidebar_col . '">
                                            <!-- =================Top Tabs================-->
                                                <div class="top_tabs">
                                                    <button class="wpf_tab_sidebar wpf_thispage wpf_active" onclick="openWPFTab(\'wpf_thispage\')" >' . __('This Page', 'atarim-visual-collaboration') . '</button>
                                                </div>
                                                <div id="sidebar_filters">
                                                    <ul class="icons-block">
                                                        <li class="status"> 
                                                            <a class="wpf_filter_tab_btn_bottom wpf_btm_withside" data-tag="wpf_task_status_filter_btn" href="javascript:void(0);" title="'.__("Filter By Status", '').'" id="wpf_filter_btn_bottom_status">'
                                                    . get_wpf_status_icon().'
                                                            </a>
                                                        </li>
                                                        <li class="priority"> <a id="wpf_filter_btn_bottom_priority" class="wpf_filter_tab_btn_bottom wpf_btm_withside" href="javascript:void(0);" data-tag="wpf_task_priority_filter_btn" title="'.__("Filter By Priority", '').'">'. get_wpf_priority_icon().'</a></li>
                                                        <li class="search">
                                                            <a id="wpf_filter_btn_bottom_search" class="wpf_filter_tab_btn_bottom wpf_btm_withside" href="javascript:void(0);" data-tag="wpf_search_filter_btn" title="'.__("Search by task title", '').'">                           
                    ...
                    $wpf_sidebar_header = '<div class="wpf_sidebar_header ' . $sidebar_col . '">
                                            <!-- =================Top Tabs================-->
                                                <div class="top_tabs">
                                                    <button class="wpf_tab_sidebar wpf_thispage wpf_active" onclick="openWPFTab(\'wpf_thispage\')" >' . __('This Page', 'atarim-visual-collaboration') . '</button>
                                                    <button class="wpf_tab_sidebar wpf_allpages"  onclick="openWPFTab(\'wpf_allpages\')" >' . __('All Pages', 'atarim-visual-collaboration') . '</button>' . $backend_btn . '<span id="close_sidebar" class="close_sidebar" onclick="expand_sidebar()">' . $wpf_sidebar_closeicon . '</span>' . '
                                                </div>
                                                <div id="sidebar_filters">
                                                    <ul class="icons-block">
                                                        <li class="status"> 
                ...
                $bottom_bar_html = '<div id="wpf_already_comment" class="wpf_hide"><div class="wpf_notice_title">' . __("Task already exist for this element.", '') . '</div><div class="wpf_notice_text">' . __("Write your message in the existing thread. <br>Here, we opened it for you.", '') . '</div></div>';
                $bottom_bar_html .= '<div id="pushed_to_media" class="wpf_hide"><div class="wpf_notice_title">' . __("Pushed to Media Folder.", '') . '</div><div class="wpf_notice_text">' . __("The file was added to the website's media folder, you can now use it from the there.", '') . '</div></div>';
                $bottom_bar_html .= '<div id="wpf_reconnecting_task" class="wpf_hide" style="display: none;"><div class="wpf_notice_title">' . __("Remapping task....", '') . '</div><div class="wpf_notice_text">' . __("Give it a few seconds. <br>Then, refresh the page to see the task in the new position.", '') . '</div></div>';
                $bottom_bar_html .= '<div id="wpf_reconnecting_enabled" class="wpf_hide" style="display: none;"><div class="wpf_notice_title">' . __("Remap task", '') . '</div><div class="wpf_notice_text">' . __("Place the task anywhere on the page to pinpoint the location of the request.", '') . '</div></div>';
                $bottom_bar_html .= '<div id="wpf_launcher" data-html2canvas-ignore="true" >
                                        <div class="wpf_launch_buttons" style="'.$bottom_button_style.'"><div class="wpf_start_comment"><a href="javascript:enable_comment();" title="' . __('Click to give your feedback!', 'atarim-visual-collaboration') . '" data-placement="left" class="comment_btn"><i class="gg-math-plus"></i></a></div><div class="wpf_expand"><a href="javascript:expand_bottom_bar()" id="wpf_expand_btn" title="' . __("Panel", '') . '"><img title="Panel" src="'. get_wpf_favicon().'" style="width:65px" alt="icon"/></a></div></div>
                                            ...
                                            <div class="filter_ui_content">
                                                <div id="wpf_side_filter">
                                                    ...
                                                    <div class="wpf_list wpf_hide" id="wpf_search_filter_btn"><div id="sidebar_search" class="wpf_search_box"><span class="wpf_search_box">
                                                        ...
                                                        <input onkeyup="wp_feedback_cat_filter(event, this)" type="text" name="wpf_search_title" class="wpf_search_title" value="" id="wpf_search_title" placeholder="'.__("Search by task title", '').'"></span></div></div>
                                                    </div>                        
                                                </div> 
                ...
                if ( get_query_var( 'is_graphic_page' ) ) {
                    ...
                    $wpf_sidebar_header = '<div class="wpf_sidebar_header ' . $sidebar_col . '">
                                            <!-- =================Top Tabs================-->
                                                <div class="top_tabs">
                                                    <button class="wpf_tab_sidebar wpf_thispage wpf_active" onclick="openWPFTab(\'wpf_thispage\')" >' . __('This Page', 'atarim-visual-collaboration') . '</button>
                                                    <button class="wpf_tab_sidebar wpf_allpages"  onclick="openWPFTab(\'wpf_allpages\')" >' . __('All Pages', 'atarim-visual-collaboration') . '</button>' . $backend_btn . '<span id="close_sidebar" class="close_sidebar" onclick="expand_compact_sidebar()">' . $wpf_sidebar_closeicon . '</span>' . '
                                                </div>
                                                <div id="sidebar_filters">
                                                    <ul class="icons-block">
                                                        <li class="status"> 
...
if ( ! function_exists( 'wp_feedback_task_status_taxonomy' ) ) {
    ...
    function wp_feedback_task_status_taxonomy() {
        $labels = array(
            'name'                       => _x( 'Task status', 'Taxonomy General Name', 'atarim-visual-collaboration' ),
            'singular_name'              => _x( 'Task status', 'Taxonomy Singular Name', 'atarim-visual-collaboration' ),
            'menu_name'                  => __( 'Task status', 'atarim-visual-collaboration' ),
            'all_items'                  => __( 'All Task status', 'atarim-visual-collaboration' ),
            'parent_item'                => __( 'Parent Item', 'atarim-visual-collaboration' ),
            'parent_item_colon'          => __( 'Parent Item:', 'atarim-visual-collaboration' ),
            'new_item_name'              => __( 'New Task status', 'atarim-visual-collaboration' ),
            'add_new_item'               => __( 'New Task status', 'atarim-visual-collaboration' ),
            'edit_item'                  => __( 'Edit Task status', 'atarim-visual-collaboration' ),
            'update_item'                => __( 'Update Task status', 'atarim-visual-collaboration' ),
            'view_item'                  => __( 'View Task status', 'atarim-visual-collaboration' ),
            'separate_items_with_commas' => __( 'Separate items with commas', 'atarim-visual-collaboration' ),
            'add_or_remove_items'        => __( 'Add or remove Task status', 'atarim-visual-collaboration' ),
            'choose_from_most_used'      => __( 'Choose from the most used', 'atarim-visual-collaboration' ),
            'popular_items'              => __( 'Popular Task status', 'atarim-visual-collaboration' ),
            'search_items'               => __( 'Search Task status', 'atarim-visual-collaboration' ),
            'not_found'                  => __( 'Not Found Task status', 'atarim-visual-collaboration' ),
            'no_terms'                   => __( 'No Task status', 'atarim-visual-collaboration' ),
            'items_list'                 => __( 'Task status list', 'atarim-visual-collaboration' ),
            'items_list_navigation'      => __( 'Task status list navigation', 'atarim-visual-collaboration' ),
        );
        $args = array(
            'labels'            => $labels,
            'hierarchical'      => true,
...
if ( ! function_exists( 'wp_feedback_task_priority_taxonomy' ) ) {	
    ...
    function wp_feedback_task_priority_taxonomy() {
        $labels = array(
            'name'                       => _x( 'Task urgency', 'Taxonomy General Name', 'atarim-visual-collaboration' ),
            'singular_name'              => _x( 'Task urgency', 'Taxonomy Singular Name', 'atarim-visual-collaboration' ),
            'menu_name'                  => __( 'Task urgency', 'atarim-visual-collaboration' ),
            'all_items'                  => __( 'All Task urgency', 'atarim-visual-collaboration' ),
            'parent_item'                => __( 'Parent Item', 'atarim-visual-collaboration' ),
            'parent_item_colon'          => __( 'Parent Item:', 'atarim-visual-collaboration' ),
            'new_item_name'              => __( 'New Task urgency', 'atarim-visual-collaboration' ),
            'add_new_item'               => __( 'New Task urgency', 'atarim-visual-collaboration' ),
            'edit_item'                  => __( 'Edit Task urgency', 'atarim-visual-collaboration' ),
            'update_item'                => __( 'Update Task urgency', 'atarim-visual-collaboration' ),
            'view_item'                  => __( 'View Task urgency', 'atarim-visual-collaboration' ),
            'separate_items_with_commas' => __( 'Separate items with commas', 'atarim-visual-collaboration' ),
            'add_or_remove_items'        => __( 'Add or remove Task urgency', 'atarim-visual-collaboration' ),
            'choose_from_most_used'      => __( 'Choose from the most used', 'atarim-visual-collaboration' ),
            'popular_items'              => __( 'Popular Task urgency', 'atarim-visual-collaboration' ),
            'search_items'               => __( 'Search Task urgency', 'atarim-visual-collaboration' ),
            'not_found'                  => __( 'Not Found Task urgency', 'atarim-visual-collaboration' ),
            'no_terms'                   => __( 'No Task urgency', 'atarim-visual-collaboration' ),
            'items_list'                 => __( 'Task urgency list', 'atarim-visual-collaboration' ),
            'items_list_navigation'      => __( 'Task urgency list navigation', 'atarim-visual-collaboration' ),
        );
        $args = array(
            'labels'            => $labels,
            'hierarchical'      => true,
